...
export function calculateMA(dayCount: number, kInterval: number, data: MinuteMarketDataEntry[], ticks = Number.MAX_SAFE_INTEGER) {
    let result = [] as number[]
    for (let i = 0, len = data.length; i < len && i < ticks; i++) {
        if (i < dayCount) {
            result.push(0);
            continue;
        }
